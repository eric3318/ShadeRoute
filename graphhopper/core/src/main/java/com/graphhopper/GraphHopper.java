...
public class GraphHopper {
  ...
  /**
   * Configures the urban density classification. Each edge will be classified as
   * 'rural','residential' or 'city', {@link UrbanDensity}
   *
   * @param residentialAreaRadius      in meters. The higher this value the longer the calculation
   *                                   will take and the bigger the area for which the road density
   *                                   used to identify residential areas is calculated.
   * @param residentialAreaSensitivity Use this to find a trade-off between too many roads being
   *                                   classified as residential (too high values) and not enough
   *                                   roads being classified as residential (too small values)
   * @param cityAreaRadius             in meters. The higher this value the longer the calculation
   *                                   will take and the bigger the area for which the road density
   *                                   used to identify city areas is calculated. Set this to zero
   *                                   to skip the city classification.
   * @param cityAreaSensitivity        Use this to find a trade-off between too many roads being
   *                                   classified as city (too high values) and not enough roads
   *                                   being classified as city (too small values)
   * @param threads                    the number of threads used for the calculation. If this is
   *                                   zero the urban density calculation is skipped entirely
   */
  public GraphHopper setUrbanDensityCalculation(double residentialAreaRadius,
   ...
   * tests. Specify storeOnFlush to true if you want that existing data will be loaded FROM disc and
   * all in-memory data will be flushed TO disc after flush is called e.g. while OSM import.
   *
  ...
  /**
   * Sets the routing profiles that shall be supported by this GraphHopper instance. The (and only
   * the) given profiles can be used for routing without preparation and for CH/LM preparation.
   * <p>
    ...
    for (Profile profile : profiles) {
      ...
      if (previous != null) {
        throw new IllegalArgumentException(
            "Profile names must be unique. Duplicate name: '" + profile.getName() + "'");
      }
    }
    return this;
  }
  ...
  /**
   * @return a mapping between profile names and according CH preparations. The map will be empty
   * before loading or import.
   */
  ...
  /**
   * @return a mapping between profile names and according landmark preparations. The map will be
   * empty before loading or import.
   */
  ...
  /**
   * Reads the configuration from a {@link GraphHopperConfig} object which can be manually filled,
   * or more typically is read from `config.yml`.
   * <p>
   * Important note: Calling this method overwrites the configuration done in some of the setter
   * methods of this class, so generally it is advised to either use this method to configure
   * GraphHopper or the different setter methods, but not both. Unfortunately, this still does not
   * cover all cases and sometimes you have to use both, but then you should make sure there are no
   * conflicts. If you need both it might also help to call the init before calling the setters,
   * because this way the init method won't apply defaults to configuration options you already
   * chose using the setters.
   */
  public GraphHopper init(GraphHopperConfig ghConfig) {
    ...
    if (ghConfig.has("routing.ch.disabling_allowed")) {
      throw new IllegalArgumentException(
          "The 'routing.ch.disabling_allowed' configuration option is no longer supported");
    }
    if (ghConfig.has("routing.lm.disabling_allowed")) {
      throw new IllegalArgumentException(
          "The 'routing.lm.disabling_allowed' configuration option is no longer supported");
    }
    if (ghConfig.has("osmreader.osm")) {
      throw new IllegalArgumentException(
          "Instead of osmreader.osm use datareader.file, for other changes see CHANGELOG.md");
    }
    ...
    countryRuleFactory =
        ghConfig.getBool("country_rules.enabled", false) ? new CountryRuleFactory() : null;
    ...
    String daTypeString = ghConfig.getString("graph.dataaccess.default_type",
        ghConfig.getString("graph.dataaccess", "RAM_STORE"));
    ...
    for (Map.Entry<String, Object> entry : ghConfig.asPMap().toMap().entrySet()) {
      if (entry.getKey().startsWith("graph.dataaccess.type.")) {
        dataAccessConfig.put(entry.getKey().substring("graph.dataaccess.type.".length()),
            entry.getValue().toString());
      }
      if (entry.getKey().startsWith("graph.dataaccess.mmap.preload.")) {
        dataAccessConfig.put(entry.getKey().substring("graph.dataaccess.mmap.".length()),
            entry.getValue().toString());
      }
    }

    if (ghConfig.getBool("max_speed_calculator.enabled", false)) {
      maxSpeedCalculator = new MaxSpeedCalculator(
    ...
    if (!ghConfig.getString("spatial_rules.location", "").isEmpty()) {
      throw new IllegalArgumentException(
          "spatial_rules.location has been deprecated. Please use custom_areas.directory instead and read the documentation for custom areas.");
    }
    if (!ghConfig.getString("spatial_rules.borders_directory", "").isEmpty()) {
      throw new IllegalArgumentException(
          "spatial_rules.borders_directory has been deprecated. Please use custom_areas.directory instead and read the documentation for custom areas.");
    }
    // todo: maybe introduce custom_areas.max_bbox if this is needed later
    if (!ghConfig.getString("spatial_rules.max_bbox", "").isEmpty()) {
      throw new IllegalArgumentException(
          "spatial_rules.max_bbox has been deprecated. There is no replacement, all custom areas will be considered.");
    }
    ...
    String customModelFolder = ghConfig.getString("custom_models.directory",
        ghConfig.getString("custom_model_folder", ""));
    setProfiles(GraphHopper.resolveCustomModelFiles(customModelFolder, ghConfig.getProfiles(),
        globalAreas));

    if (ghConfig.has("graph.vehicles")) {
      throw new IllegalArgumentException(
          "The option graph.vehicles is no longer supported. Use the appropriate turn_costs and custom_model instead, see docs/migration/config-migration-08-09.md");
    }
    if (ghConfig.has("graph.flag_encoders")) {
      throw new IllegalArgumentException(
          "The option graph.flag_encoders is no longer supported.");
    }

    encodedValuesString = ghConfig.getString("graph.encoded_values", encodedValuesString);
    dateRangeParserString = ghConfig.getString("datareader.date_range_parser_day",
    ...
    if (ghConfig.has("graph.elevation.smoothing")) {
      throw new IllegalArgumentException(
          "Use 'graph.elevation.edge_smoothing: moving_average' or the new 'graph.elevation.edge_smoothing: ramer'. See #2634.");
    }
    osmReaderConfig.setElevationSmoothing(ghConfig.getString("graph.elevation.edge_smoothing",
        osmReaderConfig.getElevationSmoothing()));
    osmReaderConfig.setSmoothElevationAverageWindowSize(
        ghConfig.getDouble("graph.elevation.edge_smoothing.moving_average.window_size",
            osmReaderConfig.getSmoothElevationAverageWindowSize()));
    osmReaderConfig.setElevationSmoothingRamerMax(
        ghConfig.getInt("graph.elevation.edge_smoothing.ramer.max_elevation",
            osmReaderConfig.getElevationSmoothingRamerMax()));
    osmReaderConfig.setLongEdgeSamplingDistance(
        ghConfig.getDouble("graph.elevation.long_edge_sampling_distance",
            osmReaderConfig.getLongEdgeSamplingDistance()));
    osmReaderConfig.setElevationMaxWayPointDistance(
        ghConfig.getDouble("graph.elevation.way_point_max_distance",
            osmReaderConfig.getElevationMaxWayPointDistance()));
    routerConfig.setElevationWayPointMaxDistance(
        ghConfig.getDouble("graph.elevation.way_point_max_distance",
            routerConfig.getElevationWayPointMaxDistance()));
    ElevationProvider elevationProvider = createElevationProvider(ghConfig);
    setElevationProvider(elevationProvider);

    if (osmReaderConfig.getLongEdgeSamplingDistance() < Double.MAX_VALUE
        && !elevationProvider.canInterpolate()) {
    ...
    if (!ghConfig.has("import.osm.ignored_highways")) {
      throw new IllegalArgumentException(
          "Missing 'import.osm.ignored_highways'. Not using this parameter can decrease performance, see config-example.yml for more details");
    }
    ...
    if ((ignoredHighwaysString.contains("footway") || ignoredHighwaysString.contains("path"))
        ...
        .anyMatch(p -> p.contains("foot") || p.contains("hike"))) {
      throw new IllegalArgumentException(
          "You should not use import.osm.ignored_highways=footway or =path in conjunction with pedestrian profiles. This is probably an error in your configuration.");
    }
    if ((ignoredHighwaysString.contains("cycleway") || ignoredHighwaysString.contains("path"))
        && ghConfig.getProfiles().stream().map(Profile::getName)
        .anyMatch(p -> p.contains("mtb") || p.contains("bike"))) {
      throw new IllegalArgumentException(
          "You should not use import.osm.ignored_highways=cycleway or =path in conjunction with bicycle profiles. This is probably an error in your configuration");
    }

    osmReaderConfig.setIgnoredHighways(Arrays.stream(
        ghConfig.getString("import.osm.ignored_highways",
                String.join(",", osmReaderConfig.getIgnoredHighways()))
            .split(",")).map(String::trim).collect(Collectors.toList()));
    osmReaderConfig.setParseWayNames(
        ghConfig.getBool("datareader.instructions", osmReaderConfig.isParseWayNames()));
    osmReaderConfig.setPreferredLanguage(ghConfig.getString("datareader.preferred_language",
        osmReaderConfig.getPreferredLanguage()));
    osmReaderConfig.setMaxWayPointDistance(ghConfig.getDouble(Routing.INIT_WAY_POINT_MAX_DISTANCE,
        osmReaderConfig.getMaxWayPointDistance()));
    osmReaderConfig.setWorkerThreads(
        ghConfig.getInt("datareader.worker_threads", osmReaderConfig.getWorkerThreads()));
    ...
    residentialAreaRadius = ghConfig.getDouble("graph.urban_density.residential_radius",
        residentialAreaRadius);
    residentialAreaSensitivity = ghConfig.getDouble("graph.urban_density.residential_sensitivity",
        residentialAreaSensitivity);
    cityAreaRadius = ghConfig.getDouble("graph.urban_density.city_radius", cityAreaRadius);
    cityAreaSensitivity = ghConfig.getDouble("graph.urban_density.city_sensitivity",
        cityAreaSensitivity);
    urbanDensityCalculationThreads = ghConfig.getInt("graph.urban_density.threads",
        urbanDensityCalculationThreads);

    // routing
    routerConfig.setMaxVisitedNodes(
        ghConfig.getInt(Routing.INIT_MAX_VISITED_NODES, routerConfig.getMaxVisitedNodes()));
    routerConfig.setTimeoutMillis(
        ghConfig.getLong(Routing.INIT_TIMEOUT_MS, routerConfig.getTimeoutMillis()));
    routerConfig.setMaxRoundTripRetries(
        ghConfig.getInt(RoundTrip.INIT_MAX_RETRIES, routerConfig.getMaxRoundTripRetries()));
    routerConfig.setNonChMaxWaypointDistance(
        ghConfig.getInt(Parameters.NON_CH.MAX_NON_CH_POINT_DISTANCE,
            routerConfig.getNonChMaxWaypointDistance()));
    routerConfig.setInstructionsEnabled(
        ghConfig.getBool(Routing.INIT_INSTRUCTIONS, routerConfig.isInstructionsEnabled()));
    int activeLandmarkCount = ghConfig.getInt(Landmark.ACTIVE_COUNT_DEFAULT,
        Math.min(8, lmPreparationHandler.getLandmarks()));
    if (activeLandmarkCount > lmPreparationHandler.getLandmarks()) {
      throw new IllegalArgumentException(
          "Default value for active landmarks " + activeLandmarkCount
              + " should be less or equal to landmark count of "
              + lmPreparationHandler.getLandmarks());
    }
  ...
  protected EncodingManager buildEncodingManager(Map<String, PMap> encodedValuesWithProps,
    ...
    List<EncodedValue> encodedValues = new ArrayList<>(activeImportUnits.entrySet().stream()
        .map(e -> {
          ...
          return f == null ? null
              : f.apply(encodedValuesWithProps.getOrDefault(e.getKey(), new PMap()));
        })
  ...
  protected OSMParsers buildOSMParsers(Map<String, PMap> encodedValuesWithProps,
    ...
    if (encodingManager.hasEncodedValue(BikeNetwork.KEY)) {
      osmParsers.addRelationTagParser(relConfig -> new OSMBikeNetworkTagParser(
          encodingManager.getEnumEncodedValue(BikeNetwork.KEY, RouteNetwork.class), relConfig));
    }
    if (encodingManager.hasEncodedValue(MtbNetwork.KEY)) {
      osmParsers.addRelationTagParser(relConfig -> new OSMMtbNetworkTagParser(
    ...
    Arrays.stream(encodedValuesStr.split(","))
        ...
        .forEach(evStr -> {
          ...
          if (encodedValuesWithProps.put(key, new PMap(evStr)) != null) {
            throw new IllegalArgumentException(
                "duplicate encoded value in config graph.encoded_values: " + key);
          }
        });
    return encodedValuesWithProps;
  }

  private static Map<String, List<String>> getRestrictionVehicleTypesByProfile(
  ...
  private static ElevationProvider createElevationProvider(GraphHopperConfig ghConfig) {
    ...
    if (ghConfig.has("graph.elevation.calcmean")) {
      throw new IllegalArgumentException(
          "graph.elevation.calcmean is deprecated, use graph.elevation.interpolate");
    }
    ...
    if (cacheDirStr.isEmpty() && ghConfig.has("graph.elevation.cachedir")) {
      throw new IllegalArgumentException(
          "use graph.elevation.cache_dir not cachedir in configuration");
    }
    ...
    if (elevationProvider instanceof TileBasedElevationProvider) {
      ...
      if (baseURL.isEmpty() && ghConfig.has("graph.elevation.baseurl")) {
        throw new IllegalArgumentException(
            "use graph.elevation.base_url not baseurl in configuration");
      }

      DAType elevationDAType = DAType.fromString(
          ghConfig.getString("graph.elevation.dataaccess", "MMAP"));
      ...
      removeTempElevationFiles = ghConfig.getBool("graph.elevation.clear",
          removeTempElevationFiles);
  ...
  private void printInfo() {
    logger.info(
        "version " + Constants.VERSION + "|" + Constants.BUILD_DATE + " (" + Constants.getVersions()
            + ")");
    if (baseGraph != null) {
  ...
  private String getBaseGraphString() {
    return encodingManager
        ...
        + "|" + (baseGraph.getTurnCostStorage() != null ? baseGraph.getTurnCostStorage()
        : "no_turn_cost")
        + "|" + getVersionsString();
  }
   ...
   * graph will be stored to disc so on a second call this method will only load the graph from disc
  ...
  protected void process(boolean closeEarly) {
    ...
    if (encodingManager == null) {
      throw new IllegalStateException(
          "The EncodingManager must be created in `prepareImport()`");
    }
  ...
  protected void prepareImport() {
    ...
    profilesByName.values().
        forEach(profile -> CustomModelParser.findVariablesForEncodedValuesString(
                profile.getCustomModel(), nameValidator, s -> "").
            forEach(var -> {
    ...
    if (!missing.isEmpty()) {
      String encodedValuesString = encodedValuesWithProps.entrySet().stream()
          .map(e -> e.getKey() + (e.getValue().isEmpty() ? ""
              : ("|" + e.getValue().toMap().entrySet().stream()
                  .map(p -> p.getKey() + "=" + p.getValue()).collect(Collectors.joining("|")))))
          .collect(Collectors.joining(", "));
      throw new IllegalArgumentException(
          "Encoded values missing: " + String.join(", ", missing) + ".\n" +
    ...
    Map<String, List<String>> restrictionVehicleTypesByProfile = getRestrictionVehicleTypesByProfile(
        profilesByName.values());
    ...
    encodingManager = buildEncodingManager(encodedValuesWithProps, activeImportUnits,
        restrictionVehicleTypesByProfile);
    osmParsers = buildOSMParsers(encodedValuesWithProps, activeImportUnits,
        restrictionVehicleTypesByProfile, osmReaderConfig.getIgnoredHighways());
  }
  ...
  protected void importOSM() {
    ...
    if (isEmpty(customAreasDirectory)) {
      ...
      logger.info(
          "Creating custom area index, reading custom areas from: '" + customAreasDirectory + "'");
    ...
    if (countryRuleFactory == null || countryRuleFactory.getCountryToRuleMap().isEmpty()) {
      ...
      logger.info("Applying rules for the following countries: {}",
          countryRuleFactory.getCountryToRuleMap().keySet());
    }

    logger.info("start creating graph from " + osmFile);
    OSMReader reader = new OSMReader(baseGraph.getBaseGraph(), osmParsers, osmReaderConfig).setFile(
            _getOSMFile()).
  ...
  private void calculateUrbanDensity() {
    if (encodingManager.hasEncodedValue(UrbanDensity.KEY)) {
      EnumEncodedValue<UrbanDensity> urbanDensityEnc = encodingManager.getEnumEncodedValue(
          UrbanDensity.KEY, UrbanDensity.class);
      if (!encodingManager.hasEncodedValue(RoadClass.KEY)) {
        throw new IllegalArgumentException(
            "Urban density calculation requires " + RoadClass.KEY);
      }
      if (!encodingManager.hasEncodedValue(RoadClassLink.KEY)) {
        throw new IllegalArgumentException(
            "Urban density calculation requires " + RoadClassLink.KEY);
      }
      EnumEncodedValue<RoadClass> roadClassEnc = encodingManager.getEnumEncodedValue(RoadClass.KEY,
          RoadClass.class);
      BooleanEncodedValue roadClassLinkEnc = encodingManager.getBooleanEncodedValue(
          RoadClassLink.KEY);
      UrbanDensityCalculator.calcUrbanDensity(baseGraph, urbanDensityEnc, roadClassEnc,
          roadClassLinkEnc, residentialAreaRadius, residentialAreaSensitivity, cityAreaRadius,
          cityAreaSensitivity, urbanDensityCalculationThreads);
    }
  }
  ...
  private List<CustomArea> readCustomAreas() {
    ...
    try (DirectoryStream<Path> stream = Files.newDirectoryStream(bordersDirectory,
        "*.{geojson,json}")) {
      for (Path borderFile : stream) {
        try (BufferedReader reader = Files.newBufferedReader(borderFile, StandardCharsets.UTF_8)) {
          JsonFeatureCollection jsonFeatureCollection = objectMapper.readValue(reader,
  ...
  private String getProfilesString() {
    return profilesByName.values().stream().map(p -> p.getName() + "|" + p.getVersion())
        .collect(Collectors.joining(","));
  }
  ...
  public void checkProfilesConsistency() {
    ...
    for (Profile profile : profilesByName.values()) {
      try {
        ...
        throw new IllegalArgumentException(
            "Could not create weighting for profile: '" + profile.getName() + "'.\n" +
                "Profile: " + profile + "\n" +
                "Error: " + e.getMessage());
      }

      if (CustomWeighting.NAME.equals(profile.getWeighting())
          && profile.getCustomModel() == null) {
        throw new IllegalArgumentException(
            "custom model for profile '" + profile.getName() + "' was empty");
      }
      if (!CustomWeighting.NAME.equals(profile.getWeighting())
          && profile.getCustomModel() != null) {
        throw new IllegalArgumentException(
            "profile '" + profile.getName() + "' has a custom model but " +
    ...
    for (CHProfile chProfile : chPreparationHandler.getCHProfiles()) {
      ...
      if (!added) {
        throw new IllegalArgumentException(
            "Duplicate CH reference to profile '" + chProfile.getProfile() + "'");
      }
      if (!profilesByName.containsKey(chProfile.getProfile())) {
        throw new IllegalArgumentException(
            "CH profile references unknown profile '" + chProfile.getProfile() + "'");
      }
    }
    Map<String, LMProfile> lmProfileMap = new LinkedHashMap<>(
        lmPreparationHandler.getLMProfiles().size());
    for (LMProfile lmProfile : lmPreparationHandler.getLMProfiles()) {
      ...
      if (previous != null) {
        throw new IllegalArgumentException(
            "Multiple LM profiles are using the same profile '" + lmProfile.getProfile() + "'");
      }
      if (!profilesByName.containsKey(lmProfile.getProfile())) {
        throw new IllegalArgumentException(
            "LM profile references unknown profile '" + lmProfile.getProfile() + "'");
      }
      if (lmProfile.usesOtherPreparation() && !profilesByName.containsKey(
          lmProfile.getPreparationProfile())) {
        throw new IllegalArgumentException("LM profile references unknown preparation profile '"
            + lmProfile.getPreparationProfile() + "'");
      }
    }
    for (LMProfile lmProfile : lmPreparationHandler.getLMProfiles()) {
      if (lmProfile.usesOtherPreparation() && !lmProfileMap.containsKey(
          lmProfile.getPreparationProfile())) {
        throw new IllegalArgumentException(
            "Unknown LM preparation profile '" + lmProfile.getPreparationProfile()
                + "' in LM profile '" + lmProfile.getProfile()
                + "' cannot be used as preparation_profile");
      }
      if (lmProfile.usesOtherPreparation() && lmProfileMap.get(lmProfile.getPreparationProfile())
          .usesOtherPreparation()) {
        throw new IllegalArgumentException("Cannot use '" + lmProfile.getPreparationProfile()
            + "' as preparation_profile for LM profile '" + lmProfile.getProfile()
            + "', because it uses another profile for preparation itself.");
      }
    }
  }
  ...
  protected void postProcessing(boolean closeEarly) {
    ...
    if (closeEarly) {
      boolean includesCustomProfiles = profilesByName.values().stream()
          .anyMatch(p -> CustomWeighting.NAME.equals(p.getWeighting()));
  ...
  void interpolateBridgesTunnelsAndFerries() {
    if (encodingManager.hasEncodedValue(RoadEnvironment.KEY)) {
      EnumEncodedValue<RoadEnvironment> roadEnvEnc = encodingManager.getEnumEncodedValue(
          RoadEnvironment.KEY, RoadEnvironment.class);
      StopWatch sw = new StopWatch().start();
      new EdgeElevationInterpolator(baseGraph.getBaseGraph(), roadEnvEnc,
          RoadEnvironment.TUNNEL).execute();
      float tunnel = sw.stop().getSeconds();
      sw = new StopWatch().start();
      new EdgeElevationInterpolator(baseGraph.getBaseGraph(), roadEnvEnc,
          RoadEnvironment.BRIDGE).execute();
      ...
      sw = new StopWatch().start();
      new EdgeElevationInterpolator(baseGraph.getBaseGraph(), roadEnvEnc,
          RoadEnvironment.FERRY).execute();
      logger.info(
          "Bridge interpolation " + (int) bridge + "s, " + "tunnel interpolation " + (int) tunnel
              + "s, ferry interpolation " + (int) sw.stop().getSeconds() + "s");
    }
  }
  ...
  protected Router createRouter() {
    ...
    return doCreateRouter(baseGraph, encodingManager, locationIndex, profilesByName,
        pathBuilderFactory,
  ...
  protected Router doCreateRouter(BaseGraph baseGraph, EncodingManager encodingManager,
      LocationIndex locationIndex, Map<String, Profile> profilesByName,
      PathDetailsBuilderFactory pathBuilderFactory, TranslationMap trMap, RouterConfig routerConfig,
      WeightingFactory weightingFactory, Map<String, RoutingCHGraph> chGraphs,
  ...
  private void calcChecksums() {
    ...
    List<Weighting> weightings = profilesByName.values().stream()
        .map(profile -> createWeighting(profile, new PMap())).toList();
  ...
  protected void loadOrPrepareCH(boolean closeEarly) {
    for (CHProfile profile : chPreparationHandler.getCHProfiles()) {
      if (!getCHProfileVersion(profile.getProfile()).isEmpty()
          && !getCHProfileVersion(profile.getProfile()).equals(
          "" + profilesByName.get(profile.getProfile()).getVersion())) {
        throw new IllegalArgumentException("CH preparation of " + profile.getProfile()
            + " already exists in storage and doesn't match configuration");
      }
    }
    ...
    Map<String, RoutingCHGraph> loaded = chPreparationHandler.load(baseGraph.getBaseGraph(),
        chConfigs);
    List<CHConfig> configsToPrepare = chConfigs.stream()
        .filter(c -> !loaded.containsKey(c.getName())).collect(Collectors.toList());
    Map<String, PrepareContractionHierarchies.Result> prepared = prepareCH(closeEarly,
        configsToPrepare);

    // we map all profile names for which there is CH support to the according CH graphs
    chGraphs = new LinkedHashMap<>();
    for (CHProfile profile : chPreparationHandler.getCHProfiles()) {
      if (loaded.containsKey(profile.getProfile()) && prepared.containsKey(
          profile.getProfile())) {
        throw new IllegalStateException(
            "CH graph should be either loaded or prepared, but not both: "
                + profile.getProfile());
      } else if (prepared.containsKey(profile.getProfile())) {
        setCHProfileVersion(profile.getProfile(),
            profilesByName.get(profile.getProfile()).getVersion());
        PrepareContractionHierarchies.Result res = prepared.get(profile.getProfile());
        chGraphs.put(profile.getProfile(),
            RoutingCHGraphImpl.fromGraph(baseGraph.getBaseGraph(), res.getCHStorage(),
                res.getCHConfig()));
      } else if (loaded.containsKey(profile.getProfile())) {
        chGraphs.put(profile.getProfile(), loaded.get(profile.getProfile()));
      } else {
        throw new IllegalStateException(
            "CH graph should be either loaded or prepared: " + profile.getProfile());
      }
    }
  }
  ...
  /**
   * For landmarks it is required to always call this method: either it creates the landmark data or
   * it loads it.
   */
  protected void loadOrPrepareLM(boolean closeEarly) {
    for (LMProfile profile : lmPreparationHandler.getLMProfiles()) {
      if (!getLMProfileVersion(profile.getProfile()).isEmpty()
          && !getLMProfileVersion(profile.getProfile()).equals(
          "" + profilesByName.get(profile.getProfile()).getVersion())) {
        throw new IllegalArgumentException("LM preparation of " + profile.getProfile()
            + " already exists in storage and doesn't match configuration");
      }
    }
    ...
    List<LMConfig> loadedConfigs = loaded.stream().map(LandmarkStorage::getLMConfig)
        .collect(Collectors.toList());
    List<LMConfig> configsToPrepare = lmConfigs.stream().filter(c -> !loadedConfigs.contains(c))
        .collect(Collectors.toList());
    ...
    for (LMProfile lmp : lmPreparationHandler.getLMProfiles()) {
      // cross-querying
      String prepProfile =
          lmp.usesOtherPreparation() ? lmp.getPreparationProfile() : lmp.getProfile();
      Optional<LandmarkStorage> loadedLMS = loaded.stream()
          .filter(lms -> lms.getLMConfig().getName().equals(prepProfile)).findFirst();
      Optional<PrepareLandmarks> preparedLMS = prepared.stream()
          .filter(pl -> pl.getLandmarkStorage().getLMConfig().getName().equals(prepProfile))
          .findFirst();
      if (loadedLMS.isPresent() && preparedLMS.isPresent()) {
        throw new IllegalStateException(
            "LM should be either loaded or prepared, but not both: " + prepProfile);
      } else if (preparedLMS.isPresent()) {
        setLMProfileVersion(lmp.getProfile(),
            profilesByName.get(lmp.getProfile()).getVersion());
        landmarks.put(lmp.getProfile(), preparedLMS.get().getLandmarkStorage());
      } else {
        loadedLMS.ifPresent(
            landmarkStorage -> landmarks.put(lmp.getProfile(), landmarkStorage));
      }
    }
  }
  ...
  protected List<PrepareLandmarks> prepareLM(boolean closeEarly, List<LMConfig> configsToPrepare) {
    ...
    return lmPreparationHandler.prepare(configsToPrepare, baseGraph, encodingManager, properties,
        locationIndex, closeEarly);
  }
  ...
  protected void cleanUp() {
    PrepareRoutingSubnetworks preparation = new PrepareRoutingSubnetworks(baseGraph.getBaseGraph(),
        buildSubnetworkRemovalJobs());
    preparation.setMinNetworkSize(minNetworkSize);
    preparation.setThreads(subnetworksThreads);
    preparation.doWork();
    logger.info("nodes: " + Helper.nf(baseGraph.getNodes()) + ", edges: " + Helper.nf(
        baseGraph.getEdges()));
  }
  ...
  private List<PrepareJob> buildSubnetworkRemovalJobs() {
    ...
    for (Profile profile : profilesByName.values()) {
      ...
      Weighting weighting = createWeighting(profile,
          new PMap().putObject(Parameters.Routing.U_TURN_COSTS, 0));
      jobs.add(
          new PrepareJob(encodingManager.getBooleanEncodedValue(Subnetwork.key(profile.getName())),
              weighting));
    }
    return jobs;
  }
  ...
  protected void flush() {
    logger.info(
        "flushing graph " + getBaseGraphString() + ", details:" + baseGraph.toDetailsString() + ", "
            + getMemInfo() + ")");
    baseGraph.flush();
    properties.flush();
  ...
  /**
   * Releases all associated resources like memory or files. But it does not remove them. To remove
   * the files created in graphhopperLocation you have to call clean().
   */
  public void close() {
    if (baseGraph != null) {
      baseGraph.close();
  ...
  public static JsonFeatureCollection resolveCustomAreas(String customAreasDirectory) {
    ...
    if (!customAreasDirectory.isEmpty()) {
      ...
      try (DirectoryStream<Path> stream = Files.newDirectoryStream(Paths.get(customAreasDirectory),
          "*.{geojson,json}")) {
        StreamSupport.stream(stream.spliterator(), false)
            .sorted(Comparator.comparing(Path::toString))
            .forEach(customAreaFile -> {
              try (BufferedReader reader = Files.newBufferedReader(customAreaFile,
                  StandardCharsets.UTF_8)) {
                globalAreas.getFeatures()
                    .addAll(mapper.readValue(reader, JsonFeatureCollection.class).getFeatures());
              } catch (IOException e) {
                throw new UncheckedIOException(e);
              }
            });
        logger.info("Will make " + globalAreas.getFeatures().size()
            + " areas available to all custom profiles. Found in " + customAreasDirectory);
      } catch (IOException e) {
        throw new UncheckedIOException(e);
      }
    }
  ...
  public static List<Profile> resolveCustomModelFiles(String customModelFolder,
      List<Profile> profiles, JsonFeatureCollection globalAreas) {
    ...
    for (Profile profile : profiles) {
      ...
      if (cm != null) {
        if (!profile.getHints().getObject("custom_model_files", Collections.emptyList())
            .isEmpty()) {
          throw new IllegalArgumentException(
              "Do not use custom_model_files and custom_model together");
        }
        try {
          ...
          throw new RuntimeException(
              "Cannot load custom_model from " + cm + " for profile " + profile.getName()
                  + ". If you are trying to load from a file, use 'custom_model_files' instead.",
              ex);
        }
      } else {
        if (!profile.getHints().getString("custom_model_file", "").isEmpty()) {
          throw new IllegalArgumentException(
              "Since 8.0 you must use a custom_model_files array instead of custom_model_file string");
        }
        List<String> customModelFileNames = profile.getHints()
            .getObject("custom_model_files", null);
        if (customModelFileNames == null) {
          throw new IllegalArgumentException(
              "Missing 'custom_model' or 'custom_model_files' field in profile '"
        ...
        if (customModelFileNames.isEmpty()) {
          ...
          for (String file : customModelFileNames) {
            if (file.contains(File.separator)) {
              throw new IllegalArgumentException(
                  "Use custom_models.directory for the custom_model_files parent");
            }
            if (!file.endsWith(".json")) {
              throw new IllegalArgumentException(
                  "Yaml is no longer supported, see #2672. Use JSON with optional comments //");
            }

            try {
              String string;
              // 1. try to load custom model from jar
              InputStream is = GHUtility.class.getResourceAsStream(
                  "/com/graphhopper/custom_models/" + file);
              ...
              if (is != null) {
                if (Files.exists(customModelFile)) {
                  throw new RuntimeException("Custom model file name '" + file
                      + "' is already used for built-in profiles. Use another name");
                }
              ...
              customModel = CustomModel.merge(customModel,
                  jsonOM.readValue(string, CustomModel.class));
            } catch (IOException ex) {
              throw new RuntimeException(
                  "Cannot load custom_model from location " + file + ", profile:"
                      + profile.getName(), ex);
            }
          }
