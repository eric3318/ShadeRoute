...
public class CustomWeighting implements Weighting {
  ...
  /**
   * Converting to seconds is not necessary but makes adding other penalties easier (e.g. turn costs
   * or traffic light costs etc)
   */
  ...
  public CustomWeighting(TurnCostProvider turnCostProvider, Parameters parameters) {
    ...
    if (this.distanceInfluence < 0) {
      throw new IllegalArgumentException(
          "distance_influence cannot be negative " + this.distanceInfluence);
    }
  }
...
/*    // add penalty at start/stop/via points
  ...
  protected double calcSeconds(double distance, EdgeIteratorState edgeState, boolean reverse) {
